...
while (Tanks.Contains(Player) && Tanks.Count > 1)
{
	...
	foreach (var tank in Tanks)
	{
		...
		if (tank.IsShooting)
		{
			// figure out where the bullet would appear
			...
			// check for an immediate wall or blockade in that cell
			bool blocked =
				// outside the playfield
				spawnX <= 0 || spawnX >= 74 ||
				spawnY <= 0 || spawnY >= 27
				// left middle blockade
				|| (5 < spawnX && spawnX < 11 && spawnY == 13)
				// top center blockade
				|| (spawnX == 37 && 3 < spawnY && spawnY < 7)
				// bottom center blockade
				|| (spawnX == 37 && 20 < spawnY && spawnY < 24)
				// right middle blockade
				|| (63 < spawnX && spawnX < 69 && spawnY == 13);

			if (!blocked)
			{
				// only spawn the bullet if there's no immediate obstacle
			...
			// reset the shooting flag and skip the rest of the update
	...
	foreach (var tank in AllTanks)
	{
		if (tank.Bullet is not null)
		{
			...
			if (collision)
			{
				if (collisionTank is not null && collisionTank.explodingFrame is 0 && --collisionTank.Health <= 0)
				{
